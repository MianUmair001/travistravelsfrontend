...
import {
  ...
  updatePlan,
  deletePlan,
} from "../../../redux/actions/plan.action";
import UpdatePlan from "../form/update_plan";

const All_plans_grid = ({ history }) => {
  const dispatch = useDispatch([]);
  const [allPlans, setAllPlans] = useState([]);

  useEffect(async () => {
    const response = await dispatch(getAllPlans());
    console.log("grid ka response", response);
  ...
  const handleEditPlan = async (e, list) => {
    e.preventDefault();
    await dispatch(
      updatePlan(
        list._id,
    ...
    history.push(`/update_plan/${list._id}`)
  };

  console.log("list hy", allPlans);
                ...
                <div id="tools">
                  <div className="row">
                    ...
                    <div className="col-md-6 col-sm-4 d-none d-sm-block text-right">
                      <Link to="/get_all_plans" className="bt_filters">
                        <i className="icon-th" />
                      </Link>{" "}
                      <Link to="all_plans_grid" className="bt_filters">
                    ...
                    <div className="col-md-6 wow zoomIn" data-wow-delay="0.1s">
                      ...
                      >
                        <Button
                          ...
                          onClick={(e) => handleEditPlan(e, list)}
                        >
                          Update
                        </Button>
                        <Button
                          ...
                          onClick={(e) => handleDeletePlan(e, list._id)}
                        >
                          Delete
                        </Button>
                      </div>
